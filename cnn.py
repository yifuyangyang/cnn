...
class SimpleCNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(c2_out, c3_out, kernel_size=3, padding=1)  # 新增第三层卷积
        ...
        # 计算全连接层输入维度
        # 经过3次池化：28x28 -> 14x14 -> 7x7 -> 3x3 (向下取整)
        # 所以第三层卷积输出的特征图尺寸为 3x3
    ...
    def forward(self, x):
        ...
        x = self.pool(x)  # -> [B, c1_out, 14, 14]
        
        x = self.relu(self.conv2(x))
        x = self.pool(x)  # -> [B, c2_out, 7, 7]
        
        x = self.relu(self.conv3(x))
        x = self.pool(x)  # -> [B, c3_out, 3, 3]
