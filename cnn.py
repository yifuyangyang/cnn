...
CONFIG = {
    ...
    # 选择要训练的模型： "mlp" 或 "cnn"
    "model": "cnn",  # 可改为 "cnn" 来训练CNN

    # 训练相关参数（可以改，用于观察收敛与精度变化）
    "epochs": 2,  # 增加训练轮数以达到更高准确率
...
class MLP(nn.Module):
    ...
    def __init__(self):
        ...
        # 优化后的MLP结构：三层隐藏层，达到98%以上准确率
        self.fc1 = nn.Linear(28 * 28, 512)   # 增加第一层神经元数量
...
class SimpleCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.relu(self.conv3(x))             # -> [B, 128, 7, 7] (conv3 applied to 4D tensor)
